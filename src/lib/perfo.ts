...
export function generatePerforation(
  ...
  // Ряды всегда расположены с шагом s.spacing по вертикали, а внутри ряда —
  // с шагом s.spacing по горизонтали (шахматное смещение сдвигает весь ряд,
  // но не уменьшает расстояние между соседями в этом ряду). Поэтому минимальное
  // расстояние между любыми соседними центрами всегда равно s.spacing —
  // независимо от stagger.
  const maxSafe = s.spacing - 0.1;
